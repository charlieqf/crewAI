...
$cmd = @"
bash -lc 'SESSION_ID="$sessionArg"; LIMIT="$Limit"; MINUTES="$Minutes";
...
curl -s "http://localhost:4096/session/${SESSION_ID}/message" | python3 -c "import json,sys; msgs=json.load(sys.stdin); users=[m for m in msgs if m.get('info',{}).get('role')=='user']; 
if not users: print('no user messages'); raise SystemExit(0);
last_user=max(users, key=lambda m: m.get('info',{}).get('time',{}).get('created') or 0);
uid=last_user.get('info',{}).get('id'); utext=''; 
for p in last_user.get('parts',[]): 
  if p.get('type')=='text': utext=p.get('text','').replace('\n',' '); break;
assist=[m for m in msgs if (m.get('info',{}).get('parentID')==uid or m.get('info',{}).get('parentId')==uid)];
max_text=''; last_text=''; last_types=[];
for m in assist:
  parts=m.get('parts',[]); types=[p.get('type') for p in parts];
  text=''.join([p.get('text','') for p in parts if p.get('type')=='text']);
  if text: last_text=text;
  if len(text)>len(max_text): max_text=text;
  last_types=types;
