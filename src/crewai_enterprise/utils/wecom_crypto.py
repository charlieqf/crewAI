...
class WeComCrypto:
    ...
    def __init__(self, token: str, encoding_aes_key: str, corp_id: str):
        ...
        if not token or not encoding_aes_key or not corp_id:
            raise WeComCryptoError("token, encoding_aes_key, and corp_id are all required")
        
        if len(encoding_aes_key) != 43:
            raise WeComCryptoError(f"EncodingAESKey must be 43 characters, got {len(encoding_aes_key)}")
    ...
    def decrypt_message(self, encrypted_msg: str) -> str:
        ...
        try:
            ...
            if received_corp_id != self.corp_id:
                raise WeComCryptoError(f"CorpID mismatch: expected {self.corp_id}, got {received_corp_id}")
